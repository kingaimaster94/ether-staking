...
const TokenData = () => {
    ...
    const onClicktotalDeposits = (event, data) => {
        const id = event.target.id;
        if (id == "tab:unstake") {
                ...
                <div className="bg-white shadow-xl rounded-md w-full sm:w-9/12 md:w-7/12 lg:w-5/12 xl:w-[40%] lg:ml-auto h-fit">
                    <Tabs className='w-full border border-primary-orange border-b-0.2 border-l-0 border-r-0 border-t-0' variant='line'>
                        <TabList>
                            ...
                            <Tab id="tab:unstake" className="rounded-r-md font-[500] text-2xl w-6/12 p-4" _selected={{ color: 'white', bg: 'rgba(249, 237, 229)', textColor: 'orange.500' }} onClick={(event) => onClicktotalDeposits(event, totalDeposits)}>UNSTAKE</Tab>
                        </TabList>
                        <TabPanels>
                            <TabPanel>
